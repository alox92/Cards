...
const updateSW = registerSW({
  onNeedRefresh() {
    ...
    if (confirm('Une nouvelle version de Cards est disponible. Installer maintenant ?')) {
      updateSW(true)
    }
  },
  onOfflineReady() {
...
// Démarrage monitor global dev (si non déjà lancé par App) + flush batch périodique
{
  ...
  userEvents.forEach(ev=> window.addEventListener(ev, ()=> {
    const now = Date.now()
    if(now - lastUserInput < 400){ consecutiveRapidInputs++ } else { consecutiveRapidInputs = 0 }
    lastUserInput = now
    if(consecutiveRapidInputs >= 2){ cancelLongWarmups() }
  }, { passive: true }))
  function shouldDefer(){ return Date.now() - lastUserInput < 500 } // si input < 500ms, on diffère
  ...
  function guarded(task: ()=>void | Promise<void>){
    ...
    return () => {
      if((shouldDefer() || pageHidden) && deferCount < MAX_DEFERS){
        deferCount++
        setTimeout(()=> PerformanceOptimizer.scheduleIdle(guarded(task), 600 + deferCount*100), 400 + deferCount*50)
        return
      }
      task()
    }
  ...
  try { await cardRepo.getById?.('__warmup__') } catch {}
    } catch {}
  }), 1200)
  ...
  PerformanceOptimizer.scheduleIdle(guarded(async ()=> {
    try {
      ...
      const forecast = container.resolve<any>(LEARNING_FORECAST_SERVICE_TOKEN)
      const ctrl1 = new AbortController(); const ctrl2 = new AbortController();
      abortRegistry.add(ctrl1); abortRegistry.add(ctrl2)
      await Promise.allSettled([
        (async()=>{ if(!ctrl1.signal.aborted) await insight?.generate?.(false) })(),
        (async()=>{ if(!ctrl2.signal.aborted) await forecast?.warmup?.() })()
      ])
      abortRegistry.delete(ctrl1); abortRegistry.delete(ctrl2)
    } catch {}
  }), 2200)
  ...
  PerformanceOptimizer.scheduleIdle(guarded(async ()=> {
    try {
  const orchestrator = container.resolve<any>(ADAPTIVE_ORCHESTRATOR_TOKEN)
  const ctrl = new AbortController(); abortRegistry.add(ctrl)
  if(!ctrl.signal.aborted){ await orchestrator?.warmup?.(ctrl.signal) }
  abortRegistry.delete(ctrl)
    } catch {}
  }), 2600)
  ...
  try { (container as any).safeResolve?.('AdaptiveOrchestratorService')?.warmup?.() } catch {}
  }, 3200)
  ...
  PerformanceOptimizer.scheduleIdle(guarded(()=> {
    ...
    try {
      if(cores>=8){
        ;(window as any).__CARDS_CPU_PROFILE__ = { poolSize: Math.min(cores-2, 6), batch: 512 }
      } else {
        ;(window as any).__CARDS_CPU_PROFILE__ = { poolSize: Math.min(cores-1, 4), batch: 256 }
      }
    } catch {}
  }), 3000)
  ...
  PerformanceOptimizer.scheduleIdle(guarded(async ()=> {
    try { await import('@/ui/components/Editor/UltraRichTextEditor') } catch {}
  }), 3400)
