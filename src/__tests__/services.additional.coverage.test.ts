...
describe("Additional service coverage", () => {
    ...
    it("scores cards with high vs low difficulty factors", () => {
      ...
      const cards = [
        ...
        card("c2", {
          ...
          easinessFactor: 2.6,
          interval: 30,
          nextReview: now + 1000 * 60 * 60 * 24 * 5,
        }),
      ];
      ...
      expect(scored[0].factors.difficulty).toBeGreaterThan(
        scored[1].factors.difficulty
      );
    });
  });
  ...
  describe("AdaptiveOrchestratorService weight adjustment", () => {
    it("adjusts weights downward quality < 0.6 and alternate path >= 0.6", async () => {
      ...
      const svc = new AdaptiveOrchestratorService(
        forecastSvc,
        insightSvc
      ) as any;
      ...
      // Low quality samples
      for (let i = 0; i < 30; i++) {
        svc.recordFeedback(0.5, 0.4, 1200);
      }
      ...
      for (let i = 0; i < 30; i++) {
        svc.recordFeedback(0.5, 0.9, 800);
      }
    ...
    it("returns cached snapshot unless force used and computes risk metrics", async () => {
      ...
      const cards = [
        ...
        },
        {
          ...
          interval: 10,
          easinessFactor: 2.8,
          lastReview: now - 86400000 * 1,
          nextReview: now + 10 * 3600_000,
          totalReviews: 8,
          correctReviews: 7,
        },
      ];
      const forecast = new LearningForecastService({
        getAll: async () => cards,
      }) as any;
  ...
  describe("CardService error paths & count fallback", () => {
    it("throws validation & wraps repo errors", async () => {
      ...
      await expect(
        svc.create("", { frontText: "a", backText: "b" } as any)
      ).rejects.toThrow("ID deck invalide ou manquant");
      await expect(
        svc.create("d1", { frontText: "", backText: "b" } as any)
      ).rejects.toThrow(/frontText doit contenir/);
      await expect(
        svc.create("d1", { frontText: "a", backText: "b" } as any)
      ).rejects.toHaveProperty("code", "OPERATION_FAILED");
      await expect(svc.update({} as any)).rejects.toThrow(/ID card/);
      await expect(svc.delete("")).rejects.toThrow(/ID card/);
      await expect(svc.get("")).rejects.toThrow(/ID card/);
  ...
  describe("DeckService stats enrichment & errors", () => {
    it("enriches stats when zero and wraps repo failures", async () => {
      ...
      const cardRepo = {
        getByDeck: vi.fn().mockResolvedValue([card("c1")]),
        deleteByDeck: vi.fn(),
        getAll: vi.fn(),
      };
      ...
      await expect(svc.createDeck({ name: "" } as any)).rejects.toThrow(
        "nom du deck doit contenir au moins 1 caractères"
      );
      await expect(
        svc.createDeck({ name: "Ok" } as any)
      ).rejects.toHaveProperty("code", "OPERATION_FAILED");
    });
  });
  ...
  describe("HeatmapStatsService large sequential fallback", () => {
    it("processes >500 sessions sequentially and returns N days array", async () => {
      ...
      const sessions = Array.from({ length: 600 }, (_, i) => ({
        startTime: now - (i % 10) * 86400000 + ((i * 1234) % 10_000),
        cardsStudied: (i % 5) + 1,
      }));
  ...
  describe("SearchIndexService instrumentation trimming", () => {
    it("trims search duration history to 200 and builds stats", async () => {
      ...
      await (globalThis as any).aribaDB.searchIndex.bulkAdd([
        { term: "alpha", cardId: "c1" },
      ]);
