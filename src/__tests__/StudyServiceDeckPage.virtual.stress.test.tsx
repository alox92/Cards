...
const STRESS_CARDS = Array.from({ length: STRESS_SIZE }, (_, i) => ({
  id: "c" + i,
  frontText:
    "Front question extrêmement longue " + i + " " + "x".repeat(i % 40),
...
// vi.mock removed in favor of vi.spyOn(container, 'resolve')
    ...
    await profile("StudyServiceDeckPage virtual stress", async () => {
      ...
      await waitFor(
        () => {
          ...
          if (!h1 || !/Deck Stress/.test(h1.textContent || ""))
            throw new Error("titre deck non chargé");
        },
        { timeout: 15000 }
      );
      ...
      const modeSelect = screen.queryByRole(
        "combobox"
      ...
      await waitFor(
        () => {
          ...
          if (count < MIN_AFTER_SCROLL)
            throw new Error(
              "pas assez rendu encore (count=" +
                count +
                " < " +
                MIN_AFTER_SCROLL +
                ")"
      ...
      await waitFor(
        () => {
          ...
          const maxIdx = Math.max(
            ...nodes.map((n) => parseInt(n.getAttribute("data-idx") || "0", 10))
          );
          if (maxIdx < 400) {
            // pousser plus loin
            attempt++;
