...
class FakeSRS {
  ...
  getStudyQueue(cards: any[], deckId: string, daily: number) {
    ...
    const fresh = cards
      .filter((c) => c.deckId === deckId && !c.due)
      .slice(0, daily);
    return [...due, ...fresh];
  }
  getBuriedIds() {
...
// Helpers
const makeCard = (id: string, deck = "d1", opts: any = {}) => ({
  id,
  deckId: deck,
...
describe("Max coverage services", () => {
  describe("AdaptiveOrchestratorService warmup & early adjust guard", () => {
    it("warmup resolves even if underlying services throw & early maybeAdjust returns", async () => {
      const forecast = {
        getForecast: vi.fn().mockRejectedValue(new Error("x")),
      } as any;
      const insight = {
        getCached: vi.fn().mockReturnValue(null),
        generate: vi.fn().mockRejectedValue(new Error("y")),
      } as any;
  ...
  describe("StudySessionService error & logging branches", () => {
    it("throws validation errors & wraps repo failure", async () => {
      ...
      await expect(svc.buildQueue("", 1)).rejects.toThrow(
      ...
      await expect(svc.buildQueue("d1", 1)).rejects.toThrow(
        "Échec StudySession buildQueue"
      );
    });
