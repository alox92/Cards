...
describe('Migration DataURL -> media refs', () => {
  it('convertit front/back DataURLs en media ids et crée les refs (skip si IndexedDB manquant)', async () => {
    ...
    // Crée carte legacy
    const card = new CardEntity({ deckId: 'd1', frontText: 'F', backText: 'B', frontImage: ONE_PIXEL, backImage: ONE_PIXEL })
    ...
    if(hasIndexedDB){
      ...
      expect(updated!.mediaRefs && updated!.mediaRefs.length).toBeGreaterThanOrEqual(2)
      const mediaRow = await mediaRepo.get(updated!.frontImage!)
      expect(mediaRow?.blob).toBeInstanceOf(Blob)
    } else {
