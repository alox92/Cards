...
describe('Final push >98% coverage', () => {
  it('AdaptiveOrchestratorService maybeAdjust both branches & normalization', async () => {
    ...
    const fbLow = Array.from({ length: 30 }, () => ({ predicted: 0.2, quality: 0.5, responseTime: 800 }))
    ...
    const fbHigh = Array.from({ length: 40 }, () => ({ predicted: 0.7, quality: 0.9, responseTime: 500 }))
    ;(orch as any).feedback = fbHigh
    ;(orch as any).lastAdjust = 0
    ;(orch as any).maybeAdjust() // shift difficulty/retention
    const w2 = orch.getWeights()
    expect(w2.difficulty + w2.retention).toBeGreaterThan(w1.difficulty + w1.retention - 0.0001)
  })

  it('SearchIndexService rebuildAll aborted path', async () => {
    // Mock minimal aribaDB tables
    const tables: any = {}
    const makeTable = () => ({ clear: async()=>{}, bulkAdd: async()=>{}, where: ()=>({ equals: ()=> ({ delete: async()=>{}, toArray: async()=>[], first: async()=>undefined }) }), count: async()=>0 })
    tables.searchIndex = makeTable(); tables.searchTrigrams = makeTable(); tables.searchTermStats = makeTable(); tables.cards = { count: async()=>0 }
    ...
    const svc = new SearchIndexService({ getAll: async()=> new Array(10).fill(0).map((_,i)=> ({ id:'c'+i, frontText:'f'+i, backText:'b'+i })) })
  ...
  it('CardService update/delete error paths', async () => {
    ...
    const repo = {
      ...
      update: async ()=> { throw new Error('fail-upd') },
      delete: async ()=> { throw new Error('fail-del') },
      getById: async ()=> null,
      getByDeck: async()=>[],
      getAll: async()=>[]
    }
    const svc = new CardService(repo as any)
    await expect(svc.update({ id:'x' } as any)).rejects.toHaveProperty('code','CARD_UPDATE_FAILED')
    await expect(svc.delete('x')).rejects.toHaveProperty('code','CARD_DELETE_FAILED')
  })
  ...
  it('DeckService error paths (getDeck not found, update & delete failures)', async () => {
    ...
    const deckRepo = {
      ...
      update: async ()=> { throw new Error('upd') },
      delete: async ()=> { throw new Error('del') }
    }
    const cardRepo = { getByDeck: async()=>[], deleteByDeck: async()=>{} }
    const svc = new DeckService(deckRepo as any, cardRepo as any)
    await expect(svc.getDeck('nope')).rejects.toHaveProperty('code','DECK_NOT_FOUND')
    await expect(svc.updateDeck({ id:'d1' } as any)).rejects.toHaveProperty('code','DECK_UPDATE_FAILED')
    await expect(svc.deleteDeck('d1')).rejects.toHaveProperty('code','DECK_DELETE_FAILED')
  })

  it('SearchService all condition branches (deckId/tag/isDue/text)', async () => {
    const { SearchService } = await import('@/application/services/SearchService')
    const now = Date.now()
    const cards:any[] = [
      { id:'a', deckId:'d1', tags:['x'], nextReview: now - 1000, frontText:'Bonjour', backText:'Monde' },
      { id:'b', deckId:'d2', tags:['y'], nextReview: now + 100000, frontText:'Hello', backText:'World' },
      { id:'c', deckId:'d1', tags:['z'], nextReview: now - 5000, frontText:'Alpha', backText:'Beta' }
    ]
  ...
  it('LearningForecastService multi-chunk & catch path', async () => {
    ...
    const badCard = { id:'err', get interval(){ throw new Error('boom') }, easinessFactor:2.5, nextReview: Date.now(), lastReview: Date.now(), totalReviews:0, correctReviews:0 }
    const many = Array.from({ length: 300 }, (_,i)=> ({ id:'c'+i, interval: 1 + (i%10), easinessFactor:2.5, nextReview: Date.now(), lastReview: Date.now()- i*3600_000, totalReviews:5, correctReviews:4 }))
    const svc = new LearningForecastService({ getAll: async()=> [badCard, ...many] })
