...
vi.mock('@/utils/logger', () => ({ logger: { error: vi.fn(), debug: vi.fn(), info: vi.fn() } }))
...
const makeTable = () => {
  const api: any = {
    ...
    put(row:any){
      const i = api._rows.findIndex((r:any)=> r.term? r.term===row.term : false)
      if(i>=0) api._rows[i]=row; else api._rows.push(row); return Promise.resolve()
    },
...
const ensureAriba = () => {
  ...
  g.aribaDB = { ...existing, ...tables, _tables: tables, table: (name: string) => (tables[name] ||= makeTable()) }
}
ensureAriba()
...
class BaseMockWorker {
  ...
  removeEventListener(type: string, cb: any){ this.listeners[type] = (this.listeners[type]||[]).filter(l=> l!==cb) }
  terminate(){}
  protected emit(type: 'message'|'error', data: any){
    if(type === 'message' && this.onmessage){ this.onmessage({ data } as any) }
  ...
  default: class HeatmapWorker extends BaseMockWorker {
    postMessage(payload: any){
      ...
      for(const s of payload.sessions){
        const d = new Date(s.startTime); const k = d.toISOString().substring(0,10)
  ...
  default: class SearchIndexWorker extends BaseMockWorker {
    postMessage(payload: any){
      ...
      for(const c of payload.cards){
        const tokens = (c.frontText + ' ' + c.backText).toLowerCase().split(/\s+/).filter(Boolean).slice(0,2)
        for(const t of tokens){ entries.push({ term: t, cardId: c.id }) }
      }
...
vi.mock('@/workers/studyQueueWorker?worker', () => ({
  default: class StudyQueueWorker extends BaseMockWorker {
    postMessage(payload: any){
      ...
      for(const c of payload.cards.slice(0, Math.min(50, payload.cards.length))){
        if(dueIds.length < 25) dueIds.push(c.id); else if(freshIds.length < payload.dailyNewLimit) freshIds.push(c.id)
      }
...
const card = (id:string, extras: any = {}) => ({ id, deckId: 'd1', frontText: 'Front '+id, backText: 'Back '+id, nextReview: Date.now()-1000, totalReviews: 0, interval: 0, ...extras })

describe('Ultra coverage – parallel & edge branches', () => {
  it('HeatmapStatsService parallel worker success path', async () => {
    const now = Date.now()
    const sessions = Array.from({ length: 800 }, (_,i)=> ({ startTime: now - (i%7)*86400000 + (i*321)%50_000, cardsStudied: (i%4)+1 }))
  ...
  it('SearchIndexService rebuildAll parallel + fuzzy worker success', async () => {
    const bigCards = Array.from({ length: 1600 }, (_,i)=> ({ id: 'c'+i, frontText: 'Alpha '+i, backText: 'Beta '+i }))
  ...
  it('StudySessionService parallel worker success (no fallback)', async () => {
    ...
    const srs = { getStudyQueue: () => { throw new Error('fallback should not be used') }, schedule: vi.fn() }
    const cardRepo = { getAll: async () => manyCards, getById: async()=>null, update: vi.fn() }
    ...
    const svc = new StudySessionService(srs as any, cardRepo as any, sessionRepo as any)
  ...
  it('PerformanceOptimizer remaining utilities (runChunked, preloadCriticalAssets, autoCleanupListener, warmupGPU, scheduleIdle idleSupported)', async () => {
    ...
    ;(globalThis as any).requestIdleCallback = (cb: any) => { cb(); return 1 }
    ...
    document.createElement = ((tag: string) => {
      ...
      if(tag==='link'){
        setTimeout(()=> { if(el.onload) el.onload(new Event('load')) },0)
      }
      return el
    }) as any
    document.head.appendChild = ((node: any) => origAppend.call(document.head, node)) as any
    ...
    let called = 0
    const cleanup = PerformanceOptimizer.autoCleanupListener(el, 'click', ()=> { called++ })
  ...
  it('AdaptiveStudyScorer edge branches (due clamp / difficulty & retention extremes)', async () => {
    ...
    const cards:any[] = [
      ...
      { id:'c2', nextReview: now - 1000, totalReviews: 5, easinessFactor: 2.3, interval: 1 } // ef <2.4 path
    ]
  ...
  it('CardService countAll optimized path + get null return', async () => {
    const repo = {
      create: vi.fn(), update: vi.fn(), delete: vi.fn(), getById: vi.fn().mockResolvedValue(null), getByDeck: vi.fn(), getAll: vi.fn().mockResolvedValue([]), count: vi.fn().mockResolvedValue(42)
    }
  ...
  it('DeckService update/delete error wrapping + validation & success branches', async () => {
    const deck = { id:'d1', name:'Deck', totalCards:1, updated: Date.now(), updateStats: vi.fn() }
    const deckRepo = {
    ...
    const cardRepo = { getByDeck: vi.fn().mockResolvedValue([]), deleteByDeck: vi.fn(), getAll: vi.fn() }
    ...
    await expect(svc.updateDeck({} as any)).rejects.toThrow(/ID deck manquant/)
    await expect(svc.updateDeck(deck as any)).rejects.toThrow(/mise à jour deck/)
    await expect(svc.deleteDeck('d1')).rejects.toThrow(/suppression deck/)
  })

  it('LearningForecastService per-card error catch branch', async () => {
    const now = Date.now()
    const badCard = { id:'b', get interval(){ throw new Error('boom') }, totalReviews:0, correctReviews:0, nextReview: now+1000 }
    const goodCard = { id:'g', interval: 2, easinessFactor:2.5, lastReview: now-86400000, nextReview: now+1000, totalReviews:2, correctReviews:2 }
    const repo = { getAll: async () => [badCard as any, goodCard] }
    const { LearningForecastService } = await import('@/application/services/LearningForecastService')
