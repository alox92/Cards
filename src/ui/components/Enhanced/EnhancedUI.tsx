...
export const MicroInteraction: React.FC<MicroInteractionProps> = ({
  ...
  const playFeedback = useCallback(() => {
    ...
    if (haptic && navigator.vibrate) {
      navigator.vibrate(intensity === 'subtle' ? 10 : intensity === 'medium' ? 20 : 40)
    }
  }, [sound, haptic, intensity])

  const handleInteraction = useCallback(async (active: boolean) => {
    setIsActive(active)
    ...
    if (active) {
      ...
      if (transitionRef.current && elementRef.current) {
        ...
        await transitionRef.current.animateIn(elementRef.current, {
          type: animationType,
          duration: intensity === 'subtle' ? 150 : intensity === 'medium' ? 250 : 350,
          easing: 'spring'
        })
      }
    }
  ...
  return (
    <div
      ref={elementRef}
      className={`micro-interaction ${type} ${intensity} ${isActive ? 'active' : ''} ${className}`}
      ...
      style={{
        transform: isActive ? `scale(${1 + getIntensityMultiplier()})` : 'scale(1)',
...
export const GlowButton: React.FC<GlowButtonProps> = ({
  ...
  return (
    <button
      className={`glow-button ${variant} ${size} ${pulse ? 'pulse' : ''} ${loading ? 'loading' : ''} ${disabled ? 'disabled' : ''} ${className}`}
      onClick={handleClick}
...
export const LiquidProgress: React.FC<LiquidProgressProps> = ({
  ...
  return (
    <div className="liquid-progress" style={{ height, backgroundColor }}>
      <div
        className={`liquid-fill ${animated ? 'animated' : ''} ${showWave ? 'wave' : ''}`}
        style={{
