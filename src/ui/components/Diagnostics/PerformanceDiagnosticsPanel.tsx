...
} from "@/application/services/LearningForecastService";
...
function usePerfHistoryFiles() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const bench = await fetch("/bench-history.json")
          .then((r) => (r.ok ? r.json() : null))
          .catch(() => null);
        const smoke = await fetch("/perf-history.json")
          .then((r) => (r.ok ? r.json() : null))
          .catch(() => null);
...
// Shape des stats FPS (réutilise FPSStats exposé par le monitor)
interface PanelState {
  fps: FPSStats;
  suppressed: { key: string; suppressed: number }[];
  open: boolean;
}
...
export const PerformanceDiagnosticsPanel: React.FC = () => {
  ...
  const [learningSeries, setLearningSeries] = useState<
    { ts: number; acc: number; mastery: number }[]
  >([]);
  ...
  const [vitalsAgg, setVitalsAgg] = useState<
    { name: string; p75: number; median: number; count: number }[]
  >([]);
  const [vitalsHi, setVitalsHi] = useState<
    { name: string; p95: number; p99: number }[]
  >([]);
  const [vitalsHist, setVitalsHist] = useState<
    { name: string; buckets: { b: number; c: number }[] }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // @ts-ignore
        const rows = await (aribaDB as any).searchTermStats
          .orderBy("count")
          .reverse()
          .limit(10)
          .toArray()
  ...
  useEffect(() => {
    let mounted = true;
    const svc = container.resolve<LearningForecastService>(
      LEARNING_FORECAST_SERVICE_TOKEN as any
    );
    const pull = async () => {
      try {
        const snap = await svc.getForecast();
        if (mounted)
          setForecast({
            avg: snap.averageRisk,
            high: snap.highRiskCount,
            ts: snap.generatedAt,
            items: snap.items.slice(0, 5),
          });
      } catch {}
    };
    pull();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch("/learning-history.json")
          .then((r) => (r.ok ? r.json() : null))
          .catch(() => null);
        if (res && Array.isArray(res) && mounted) {
          setLearningSeries(
            res
              .filter((x: any) => typeof x.accuracy === "number")
              .map((x: any) => ({
                ts: x.ts,
                acc: x.accuracy,
                mastery: x.mastery ?? 0,
  ...
  useEffect(() => {
    ...
    if (p && learningSeries.length) {
      // n'ajouter qu'après premier chargement fichier
      setLearningSeries((s) => [
        ...s.slice(-199),
        {
          ts: Date.now(),
          acc: p.performance.overallAccuracy,
          mastery: p.performance.masteryLevel,
        },
      ]);
    }
  }, [
    learning.profile?.performance.overallAccuracy,
    learning.profile?.performance.masteryLevel,
  ]);
    ...
    const id = setInterval(() => {
      ...
      try {
        ...
        try {
          ...
          const histArr = Object.entries(histStore).map(
            ([name, buckets]: any) => {
              const list = Object.entries(buckets)
                .map(([b, c]: any) => ({ b: Number(b), c: c as number }))
                .sort((a, b) => a.b - b.b);
      ...
      try {
        const sched = (window as any).__PERF_SCHED__;
        if (sched) setSchedulerInfo(sched);
      } catch {
        /* ignore */
      }
      try {
        const ss = (window as any).__SEARCH_DURS_STATS__;
        if (ss) setSearchLatencyStats(ss);
      } catch {
        /* ignore */
      }
    }, 1500);
        ...
        <div className="mt-2 w-80 max-h-[60vh] overflow-auto rounded-lg bg-gray-900/90 backdrop-blur text-gray-100 shadow-lg border border-gray-700 p-3 space-y-2">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-2 py-0.5 rounded bg-indigo-600/70 hover:bg-indigo-600"
              >
                Flush
              </button>
              <button
                ...
                className="px-2 py-0.5 rounded bg-gray-600/70 hover:bg-gray-600"
              >
                Reset
              </button>
              <button
                onClick={async () => {
                  try {
                    ...
                    const insightSvc: any =
                      (container as any).safeResolve?.("InsightService") ||
                      container.resolve?.("InsightService" as any);
                    const forecastSvc: any =
                      (container as any).safeResolve?.(
                        "LearningForecastService"
                      ) ||
                      container.resolve?.("LearningForecastService" as any);
                    insightSvc?.resetCache?.();
                    forecastSvc?.clearSnapshot?.();
                    setForecast({});
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const searchSvc: any =
                      (container as any).safeResolve?.("SearchIndexService") ||
                      container.resolve?.("SearchIndexService" as any);
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-[10px]">
            <div>
              ...
              <div className="text-white font-semibold">{state.fps.avg}</div>
            </div>
            <div>
              <div className="text-gray-400">MIN</div>
              <div className="text-white font-semibold">{state.fps.min}</div>
            </div>
            <div>
              <div className="text-gray-400">MAX</div>
              <div className="text-white font-semibold">{state.fps.max}</div>
            </div>
            <div>
              <div className="text-gray-400">WARN</div>
            ...
            <div>
              ...
              <div className="text-red-400 font-semibold">
                {state.fps.consecutiveLow}
              </div>
            </div>
            <div>
              <div className="text-gray-400">ADAPT</div>
              <div className="text-blue-400 font-semibold">
                {state.fps.adaptiveApplied}
              </div>
            </div>
          </div>
          {schedulerInfo && (
            <div className="grid grid-cols-4 gap-1 text-center text-[9px] mt-1">
              <div>
                <div className="text-gray-500">Q-C</div>
                <div className="text-white font-semibold">
                  {schedulerInfo.queues?.critical}
                </div>
              </div>
              <div>
                <div className="text-gray-500">Q-H</div>
                <div className="text-white font-semibold">
                  {schedulerInfo.queues?.high}
                </div>
              </div>
              <div>
                <div className="text-gray-500">Q-N</div>
                <div className="text-white font-semibold">
                  {schedulerInfo.queues?.normal}
                </div>
              </div>
              <div>
                <div className="text-gray-500">Q-B</div>
                <div className="text-white font-semibold">
                  {schedulerInfo.queues?.background}
                </div>
              </div>
              <div className="col-span-4 flex justify-between text-[9px] text-gray-400">
                ...
                <span>
                  Last:{Math.round(schedulerInfo.stats?.lastExecDur || 0)}ms
                </span>
              </div>
            </div>
          )}
          ...
          {metrics && (
            <div className="grid grid-cols-3 gap-2 text-center text-[10px] border-t border-gray-700 pt-2">
              ...
              <div>
                <div className="text-gray-400">WQ</div>
                <div className="text-white font-semibold">
                  {metrics.workerQueueLength ?? 0}
                </div>
              </div>
            </div>
          )}
          {history && history.length > 5 && (
            <div className="h-6 w-full relative">
              ...
              <div className="absolute top-0 left-0 text-[9px] text-gray-500">
                Mem
              </div>
            </div>
          )}
          {lastOpt && (
            <div className="text-[10px] text-gray-400 flex gap-2 border-t border-gray-700 pt-1">
              ...
              <span>
                {Math.round((Date.now() - lastOpt.timestamp) / 1000)}s
              </span>
            </div>
          )}
          ...
          {state.fps.adaptationsHistory &&
            state.fps.adaptationsHistory.length > 0 && (
                ...
                {state.fps.adaptationsHistory
                  ...
                  .map((a, i) => (
                    <div key={i} className="flex justify-between">
                      <span>{Math.round(a.ts / 1000)}s</span>
                      <span className="text-blue-300">→ {a.newWarnBelow}</span>
                    </div>
                  ))}
              </div>
            )}
          ...
          {external.bench &&
            Array.isArray(external.bench) &&
            external.bench.length > 0 && (
              <div className="border-t border-gray-700 pt-1">
                <div className="text-[10px] text-gray-300 font-semibold mb-0.5">
                  Bench ops (sm2 single)
                </div>
                {(() => {
                  const last = external.bench.slice(-30);
                  const series = last
                    .map((s: any) => {
                      const b = (s.benches || []).find((x: any) =>
                        x.name.includes("sm2Update single")
                      );
                      return b ? b.ops : null;
                    })
                    .filter((v: any) => v != null);
          ...
          {external.smoke &&
            Array.isArray(external.smoke) &&
            external.smoke.length > 0 && (
              <div className="border-t border-gray-700 pt-1">
                <div className="text-[10px] text-gray-300 font-semibold mb-0.5">
                  Smoke buildQueueMs
                </div>
          ...
          {learning.recommendations.length > 0 && (
            <div className="border-t border-gray-700 pt-1 max-h-28 overflow-auto">
              ...
              {learning.recommendations.slice(0, 5).map((r) => (
                <div
                  key={r.action}
                  className="text-[10px] flex justify-between gap-2 opacity-90"
                >
                  <span className="truncate">{r.title}</span>
                  <span className="text-gray-400">
                    {Math.round(r.estimatedBenefit * 100)}%
                  </span>
                </div>
              ))}
            </div>
              ...
              <div className="grid grid-cols-3 text-center text-[10px] mb-1">
                ...
                <div>
                  ...
                  <div className="text-gray-300">
                    {forecast.ts
                      ? Math.round((Date.now() - forecast.ts) / 1000)
                      : "–"}
                    s
                  </div>
                </div>
            ...
            <div className="border-t border-gray-700 pt-1">
              ...
              <div className="grid grid-cols-3 gap-1 text-[9px] mb-1">
                {vitals.map((m: any, i) => (
                  <div
                    key={i}
                    className="flex flex-col bg-gray-800/60 rounded p-1"
                  >
                ...
                <div className="space-y-0.5 max-h-24 overflow-auto">
                  ...
                  {vitalsHi.length > 0 &&
                    vitalsHi.map((h) => (
                      <div
                        key={h.name + ":hi"}
                        className="flex justify-between text-[9px] text-gray-500"
                      >
                  ...
                  {vitalsHist.length > 0 &&
                    vitalsHist.map((h) => (
                      <div
                        key={h.name + ":hist"}
                        className="text-[8px] text-gray-500 flex items-center gap-1"
                      >
                          ...
                          {h.buckets.slice(0, 10).map((b) => {
                            ...
                            return (
                              <span
                                ...
                                title={`${b.b} (${b.c})`}
                                style={{ height }}
                                className="w-1 bg-gray-600 inline-block"
                              ></span>
                            );
                          })}
                        </span>
            ...
            <div className="border-t border-gray-700 pt-1">
              ...
              <div className="grid grid-cols-4 text-center text-[9px] mb-1">
                <div>
                  ...
                  <div className="text-white">{searchLatencyStats.count}</div>
                </div>
                <div>
                  <div className="text-gray-500">P50</div>
                  <div className="text-white">
                    {Math.round(searchLatencyStats.p50)}
                  </div>
                </div>
                <div>
                  <div className="text-gray-500">P95</div>
                  <div className="text-orange-400">
                    {Math.round(searchLatencyStats.p95)}
                  </div>
                </div>
                <div>
                  <div className="text-gray-500">P99</div>
                  <div className="text-pink-400">
                    {Math.round(searchLatencyStats.p99)}
                  </div>
                </div>
              </div>
              <div className="flex items-end gap-0.5 h-4">
                {Object.entries(searchLatencyStats.hist)
                  ...
                  .map(([b, c]: any) => (
                    <div
                      key={b}
                      title={`${b}ms : ${c}`}
                      style={{ height: Math.max(2, Math.min(16, c * 2)) }}
          ...
          <div className="text-[10px] text-gray-400 flex gap-2 flex-wrap">
            <span>
              {state.fps.running ? (state.fps.paused ? "⏸️" : "▶️") : "■"}
            </span>
              ...
              onClick={() => {
                try {
                  ...
                  logger.debug(
                    ...
                    "Perf snapshot uploaded (debug array)",
                    { count: arr.length, last: snapshot }
                  );
                } catch {
                  /* ignore */
                }
          ...
          <div>
            <h4 className="text-[11px] font-semibold mt-2 mb-1">
              Logs supprimés
            </h4>
            {state.suppressed.length === 0 && (
              <div className="text-[10px] text-gray-500">Aucun</div>
            )}
            {state.suppressed.slice(0, 15).map((s) => (
              <div
                key={s.key}
                className="truncate text-[10px] flex justify-between gap-2"
              >
          ...
          <div className="text-[10px] text-gray-500">
            Panel expérimental – gouverné par feature flag.
          </div>
        </div>
      )}
    </div>
