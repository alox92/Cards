...
export const DebugTestPage: React.FC = () => {
  ...
  const runLogTests = async () => {
    ...
    logger.trace("Test", "Message de trace - détail technique", {
      data: "trace",
    });
    logger.debug("Test", "Message de debug - information développeur", {
      debug: true,
    });
    logger.info("Test", "Message informatif - événement important", {
      event: "test",
    });
    ...
    try {
      throw new Error(
        "Erreur de test pour vérifier la capture des stack traces"
      );
    } catch (error) {
  ...
  const refreshLogs = () => {
    ...
    try {
      ...
      if (storedLogs) {
        const parsedLogs = JSON.parse(storedLogs) as DebugLogEntry[];
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-mono">
                  {performanceMetrics.completedOperations || 0}
                </span>
              </div>
              <div>
                <span className="text-blue-600 dark:text-blue-400">
                ...
                <span className="ml-2 font-mono">
                  {performanceMetrics.averageDuration || 0}ms
                </span>
              </div>
              <div>
                <span className="text-blue-600 dark:text-blue-400">
            ...
            <div className="space-y-2">
              {logs.map((log, index) => (
                <motion.div
                  ...
                  className={`p-3 rounded-lg font-mono text-sm ${getLevelColor(
                    log.level
                  )}`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <span className="font-bold min-w-[60px]">{log.level}</span>
      ...
      >
        ...
        <ul className="space-y-2 text-amber-800 dark:text-amber-200">
          <li>
            • <strong>Lancer Tests:</strong> Exécute une série de tests pour
            tous les niveaux de logging
          </li>
          <li>
            • <strong>Rafraîchir:</strong> Met à jour l'affichage des logs
            depuis le stockage local
          </li>
          <li>
            • <strong>Exporter:</strong> Télécharge tous les logs dans un
            fichier JSON
          </li>
          <li>
            • <strong>Effacer:</strong> Supprime tous les logs stockés
            (irréversible)
          </li>
          <li>
            • <strong>Console:</strong> Ouvrez F12 pour voir les logs colorés
            dans la console
          </li>
          <li>
            • <strong>Performance:</strong> Les métriques incluent les timers et
            l'usage mémoire
          </li>
        </ul>
      </motion.div>
