...
export default function CardEditorServicePage() {
  ...
  if(deckId && !deckName) {
    deckService.getDeck(deckId).then(d => { if(d) setDeckName(d.name) })
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Nouvelle carte (services)</h1>
            {deckName && <p className="text-sm text-gray-500 dark:text-gray-400">Deck: {deckName}</p>}
          </div>
          <div className="flex gap-3">
            <button onClick={()=>navigate(-1)} className="btn-secondary">Annuler</button>
            <button disabled={isLoading || !frontText.trim() || !backText.trim()} onClick={save} className="btn-primary">{isLoading? 'Enregistrement...' : 'Enregistrer'}</button>
          </div>
        </div>
        ...
        <div className="card space-y-6">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400">L'intervalle initial sera ajusté automatiquement après vos premières réponses.</div>
        </div>
      </div>
    </div>
  )
