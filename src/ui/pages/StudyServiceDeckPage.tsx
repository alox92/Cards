...
import {
  ...
  useDeferredSearch,
} from "@/utils/reactConcurrentFeatures";
...
const StudyServiceDeckPage: React.FC = () => {
  ...
  }, [deckId, deckService, cardService]); // executeTransition is stable
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      alert("Échec création carte: " + (e?.message || "inconnu"));
    } finally {
      setCreating(false);
    }
  };
      ...
      {isEditing ? (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={onSaveEdit} className="px-3 py-1 bg-green-600 text-white rounded text-sm flex items-center gap-1">
              <Icons.Check size="xs" />
            </button>
            <button onClick={onCancelEdit} className="px-3 py-1 bg-gray-600 text-white rounded text-sm flex items-center gap-1">
              <Icons.X size="xs" />
            </button>
          </div>
        </div>
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex gap-1">
              <button
                ...
                className="text-xs px-2 py-1 rounded bg-gray-200 hover:bg-gray-300 flex items-center gap-1"
              >
                {isSelected ? <Icons.Check size="xs" /> : <span className="w-3 h-3 border-2 border-gray-400 rounded"></span>}
              </button>
              <button
                onClick={() => onStartEdit(card)}
                className="text-xs px-2 py-1 rounded bg-blue-200 hover:bg-blue-300 flex items-center gap-1"
              >
                <Icons.Edit size="xs" />
              </button>
              <button
                onClick={() => onDelete(card.id!)}
                className="text-xs px-2 py-1 rounded bg-red-200 hover:bg-red-300 flex items-center gap-1"
              >
...
*/
  ...
  const virtualize =
    mode === "virtual" && filtered.length > VIRTUALIZE_THRESHOLD;
  const paginate =
    mode === "pagination" && filtered.length > VIRTUALIZE_THRESHOLD;
  const PAGE_SIZE = 200;
  const OVERSCAN = 10;
  ...
  const startEdit = (c: CardEntity) => {
    setEditingId(c.id!);
    setEditFront(c.frontText);
    setEditBack(c.backText);
  };
  const cancelEdit = () => {
    setEditingId(null);
    setEditFront("");
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      alert("Échec sauvegarde: " + (e?.message || "inconnu"));
    }
  };
  const deleteCard = async (id: string) => {
    ...
    try {
      ...
      alert("Échec suppression: " + (e?.message || "inconnu"));
    }
  };

  // Mini étude sélection
  const selectionArray = Array.from(selectedIds);
  const currentStudyCard = studySelectionActive
    ? cards.find((c) => c.id === selectionArray[studySelIndex])
    : null;
  const nextStudy = () =>
    setStudySelIndex((i) => Math.min(selectionArray.length - 1, i + 1));
  const prevStudy = () => setStudySelIndex((i) => Math.max(0, i - 1));
  const closeStudy = () => {
    setStudySelectionActive(false);
    setStudySelIndex(0);
  };
  ...
  useEffect(() => {
    // scroll auto élément sélectionné (approx via scrollTop cible)
    ...
    <FuturisticLayout>
          ...
          <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
            <div>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {cards.length} cartes • Vue liste rapide{" "}
                {virtualize && `(virtualisation)`}
                {paginate && `(pagination)`}
              </div>
            </div>
            <div className="flex gap-2">
              {filtered.length > VIRTUALIZE_THRESHOLD && (
                <select
                  ...
                  className="px-2 py-1 text-xs rounded border bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600"
                >
              ...
              >
                {showBack ? "Cacher verso" : "Afficher verso"}
              </button>
              <button
                onClick={() => load()}
                disabled={loading}
              ...
              <button
                onClick={() => setStudySelectionActive(true)}
                disabled={selectedIds.size === 0}
                className="px-3 py-1 text-sm rounded bg-green-600 text-white hover:bg-green-700 disabled:opacity-40"
              >
                Étudier sélection ({selectedIds.size})
              </button>
              {selectedIds.size > 0 && (
                <button
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-1 space-y-4">
              <div className="card p-4">
                <h2 className="font-semibold mb-2 text-gray-800 dark:text-gray-100 flex items-center gap-2">
                  <Icons.Add size="sm" />
                ...
                <input
                  ...
                  placeholder="Recto"
                  className="w-full mb-2 px-2 py-1 rounded border text-sm bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600"
                />
                <textarea
                  value={back}
                ...
                <button
                  ...
                  className="w-full py-1.5 rounded bg-green-600 text-white hover:bg-green-700 disabled:opacity-50 text-sm"
                >
              ...
              <div className="card p-4">
                ...
                <input
                  ...
                  placeholder="texte..."
                  className="w-full px-2 py-1 rounded border text-sm bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600"
                />
              ...
              <div className="card p-3 text-xs text-gray-500 dark:text-gray-400">
                Base route:{" "}
                <Link to="/study-service" className="underline">
                  liste des decks
                </Link>
              </div>
            </div>
            <div className="md:col-span-2">
              <div
                ...
                data-mode={
                  virtualize ? "virtual" : paginate ? "pagination" : "plain"
                }
              ...
              >
                ...
                {visibleItems.map((c, localIdx) => {
                  const globalIndex = virtualize
                    ? startIndex + localIdx
                    : paginate
                    ? page * PAGE_SIZE + localIdx
                    : localIdx;
                  const selected = selectedIds.has(c.id!);
                  const isEditing = editingId === c.id;
                  const isCurrent = globalIndex === selectedIndex;
                  const ref =
                    virtualize && localIdx === 0 ? firstItemRef : undefined;
                  return (
                    <div
                      key={c.id}
                      ...
                      className={`px-3 py-2 cursor-pointer focus:outline-none ${
                        isCurrent
                          ? "bg-blue-50 dark:bg-blue-900/30"
                          : "hover:bg-gray-50 dark:hover:bg-gray-800/60"
                      }`}
                      style={
                        virtualize ? { minHeight: itemHeight * 0.6 } : undefined
                      }
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          {!isEditing && (
                            <>
                              <div className="font-medium text-sm text-gray-900 dark:text-gray-100 whitespace-pre-wrap break-words">
                                {c.frontText}
                              </div>
                              {showBack && (
                          ...
                          {isEditing && (
                            <div className="space-y-1">
                              <input
                                value={editFront}
                                onChange={(e) => setEditFront(e.target.value)}
                                className="w-full px-2 py-1 rounded border text-xs bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600"
                              />
                              <textarea
                                value={editBack}
                                onChange={(e) => setEditBack(e.target.value)}
                              ...
                              <div className="flex gap-2 text-xs mt-1">
                                ...
                                <button
                                  ...
                                  className="px-2 py-0.5 rounded bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500"
                                >
                                  Annuler
                                </button>
                              </div>
                          ...
                          {c.tags && c.tags.length > 0 && !isEditing && (
                            <div className="mt-1 flex flex-wrap gap-1">
                              {c.tags.slice(0, 6).map((t) => (
                                <span
                                  key={t}
                                  className="px-1 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-[10px] text-gray-600 dark:text-gray-300"
                                >
                                  {t}
                                </span>
                              ))}
                        ...
                        <div className="flex flex-col gap-1 ml-2 pt-0.5">
                          {!isEditing && (
                            <button
                              ...
                              className="text-[10px] px-1 py-0.5 rounded bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600"
                            >
                              ✎
                            </button>
                          )}
                          {!isEditing && (
                            <button
                              ...
                              className="text-[10px] px-1 py-0.5 rounded bg-red-500 text-white hover:bg-red-600"
                            >
                              ✖
                            </button>
                          )}
      ...
      {deckId && (
        <button
          ...
          className="fixed bottom-6 right-6 h-14 w-14 rounded-full bg-gradient-to-br from-blue-600 to-sky-500 text-white shadow-lg flex items-center justify-center hover:scale-105 active:scale-95 transition z-40"
          ...
          <div className="bg-white dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <div className="text-base font-medium text-gray-700 dark:text-gray-200">
                Étude sélection ({studySelIndex + 1}/{selectionArray.length})
              </div>
              <div className="flex gap-2 items-center">
                <button
                  onClick={() => setShowBack((b) => !b)}
                  className="px-3 py-2 text-sm rounded bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 transition"
                >
                  Flip
                </button>
                <button
                  onClick={closeStudy}
                  className="px-3 py-2 text-sm rounded bg-red-500 text-white hover:bg-red-600 transition"
                >
                  Fermer
                </button>
              </div>
            ...
            <div className="flex-1 overflow-auto p-10">
              {currentStudyCard ? (
                <div className="relative">
                  <div className="mb-6 text-center text-2xl font-semibold text-gray-900 dark:text-gray-100 whitespace-pre-wrap break-words leading-relaxed">
                    {currentStudyCard.frontText}
                  </div>
                  {showBack && (
                    <div className="mt-6 p-6 rounded-lg bg-gray-100 dark:bg-gray-800 text-lg text-gray-800 dark:text-gray-200 whitespace-pre-wrap break-words leading-relaxed">
            ...
            <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <div className="flex gap-3">
                <button
                  onClick={prevStudy}
                  disabled={studySelIndex === 0}
                  className="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 disabled:opacity-40 hover:bg-gray-300 dark:hover:bg-gray-600 transition text-sm"
                >
                  ←
                </button>
                <button
                  onClick={nextStudy}
                  disabled={studySelIndex >= selectionArray.length - 1}
                  className="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 disabled:opacity-40 hover:bg-gray-300 dark:hover:bg-gray-600 transition text-sm"
                >
                  →
                </button>
