...
import type { StudySession, StudyMode } from "@/domain/entities/StudySession";
...
export function useServiceStudySession({
  deckId,
  dailyNewLimit = 15,
  mode = "quiz",
  enabled = true,
  ...
  const buildQueue = useCallback(async () => {
    ...
    try {
      ...
      setError(e.message || "Erreur construction session");
    } finally {
      setLoading(false);
    }
  }, [deckId, dailyNewLimit, service, mode, enabled]);
  ...
  }, [deckId, dailyNewLimit, service, enabled]);
    ...
    async (quality: number) => {
      ...
      try {
        ...
        if (sessionRef.current) {
          ...
          // recalcul simple
          sessionRef.current.averageResponseTime = sessionRef.current
            .cardsStudied
            ? (sessionRef.current.averageResponseTime *
                (sessionRef.current.cardsStudied - 1) +
                elapsed) /
              sessionRef.current.cardsStudied
            : 0;
        }
        setQueue((q) => {
        ...
        if (queue.length === 1) {
          ...
          if (sessionRef.current) {
            ...
            setFinished(true);
            try {
              localStorage.removeItem(`cards.activeSession.${deckId}`);
            } catch {
              /* ignore */
            }
          }
        ...
        setError(e.message || "Erreur réponse");
      } finally {
        setAnswering(false);
      }
    },
