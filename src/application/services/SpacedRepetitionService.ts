  ...
  schedule(
    ...
    try {
      if (quality < 0 || quality > 5) {
        logger.warn("SpacedRepetitionService", "Quality hors intervalle 0-5", {
          quality,
        });
        return err({
          code: "SRS_QUALITY_RANGE",
          message: "Quality hors intervalle 0-5",
        });
      }
      card.recordResponse(quality, responseTimeMs);
      // Leech detection
      if (
        card.totalReviews >= this.leechThreshold &&
        card.correctReviews / card.totalReviews < 0.5
      ) {
      ...
      logger.error(
        ...
        "schedule: Erreur planification",
        e
      );
      return err({
        code: "SRS_SCHEDULE_FAILED",
        message: "Erreur planification",
      });
    }
  }
  getStudyQueue(
    ...
    try {
      ...
      logger.error(
        "SpacedRepetitionService",
        "getStudyQueue: Erreur construction queue",
        e
      );
      return err({
        code: "SRS_QUEUE_FAILED",
        message: "Erreur construction queue",
      });
    }
  }
  bury(cardIds: string[]) {
    cardIds.forEach((id) => this.buriedToday.add(id));
  }
  resetBuried() {
    this.buriedToday.clear();
  }
  /** Expose read-only list of buried card IDs for advanced filtering */
  getBuriedIds(): string[] {
    return Array.from(this.buriedToday);
  }
}
