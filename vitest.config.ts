...
const TEST_SUITE =
  process.env.TEST_SUITE || (process.env.FAST_TESTS === "1" ? "fast" : "heavy");
...
export default defineConfig({
  ...
  test: {
    ...
    exclude: [
      ...
      IS_FAST ? "src/**/*.heavy.test.tsx" : "",
    ].filter(Boolean),
    // Nouveau: reporters conditionnels (JUnit en CI)
    reporters: USE_JUNIT
      ? ["default", ["junit", { outputFile: "coverage/junit-report.xml" }]]
      : ["default"],
    // Réduction bruit logs en mode fast (ignore debug/traces verbeux)
    onConsoleLog(log) {
      if (IS_FAST && (log.includes("[debug]") || log.includes("[trace]")))
        return false;
    },
